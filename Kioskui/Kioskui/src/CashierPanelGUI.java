...
public class CashierPanelGUI extends JFrame {
    ...
    public CashierPanelGUI(ProductManager product, EquipmentManager equipment, MembershipManager membership) {
        ...
        ActionListener showByCategory = e -> {
            ...
            for (Product p : productManager.getProducts()) {
                switch (cmd) {
                    case "Food" -> {
                        if (p instanceof Food) itemListModel.addElement(p);
                    }
                    case "Drink" -> {
                        if (p instanceof Drink) itemListModel.addElement(p);
                    }
                    case "Dessert" -> {
                        if (p instanceof Dessert) itemListModel.addElement(p);
                    }
                }
            }
        };
